...
def prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["p_long_smooth","p_short_smooth","p_none_smooth","top_class","realized_dir","correct"]:
    ...
    # numeric coercion
    for c in ["p_long_smooth","p_short_smooth","p_none_smooth","correct"]:
        df[c] = coerce_num(df[c], 0.0)

    # recompute argmax, top prob, and delta from smoothed probs
    ...
    # store in frame for reuse
    df["__top_idx"] = top_idx
    df["__top_prob"] = top_prob
    df["__delta_gap"] = delta_gap

    # normalize correct to int
    df["correct"] = coerce_num(df["correct"], 0).astype(int)

    # predicted class derived from smoothed probs
    idx_to_class = np.array(["Long","Short","None"])
    df["__predicted"] = idx_to_class[top_idx]

    # If top_class column differs, we prefer recomputed predicted class
    return df
...
def eval_metrics(df: pd.DataFrame, p: float, d: float) -> dict:
    ...
    # accuracy based on whether predicted equals realized. If 'correct' already encoded, use it, but only for accepted rows
    # If correct is not reliable, recompute from strings
    if "realized_dir" in df.columns:
        acc_vec = (acc_df["__predicted"].astype(str) == acc_df["realized_dir"].astype(str)).astype(int)
        accuracy_on_accepted = acc_vec.mean() if acc_rows else 0.0
    else:
        accuracy_on_accepted = acc_df["correct"].mean() if acc_rows else 0.0

    # PF with 1 to 1 RRR, accepted rows only, only Long or Short contribute
    longs_or_shorts = acc_df["__predicted"].isin(["Long","Short"])
    pnl = np.where(longs_or_shorts & (acc_df["__predicted"] == acc_df["realized_dir"]), 1,
                   np.where(longs_or_shorts & (acc_df["__predicted"] != acc_df["realized_dir"]), -1, 0))
    gains = pnl[pnl > 0].sum()
...
def grid_search(df: pd.DataFrame, p_min=0.50, p_max=0.70, p_step=0.02,
                d_min=0.08, d_max=0.16, d_step=0.02):
...
def main():
    ap = argparse.ArgumentParser(description="Threshold tuning for EA_3_0_TCN. Recomputes acceptance from smoothed probs.")
    ...
    grid_path = os.path.join(args.outdir, "step3_threshold_grid.csv")
    grid.to_csv(grid_path, index=False)

    # top 5 by PF, break ties by accepted_rows descending
    top5 = grid.sort_values(["pf_rr1","accepted_rows"], ascending=[False, False]).head(5)
    top5_path = os.path.join(args.outdir, "step3_top5.csv")
    top5.to_csv(top5_path, index=False)

    # make a heatmap for PF
    heatmap_path = os.path.join(args.outdir, "step3_pf_heatmap.png")
